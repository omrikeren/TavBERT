...
class MaskTokensDataset(BaseWrapperDataset):
    ...
    def __init__(
        ...
        print("vocabulary:", vocab.indices)
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            masked_inds = set()
            while len(masked_inds) < num_mask:
                span_len = min(num_mask - len(masked_inds), np.random.poisson(lam=self.poisson_lambda))
                start_pos = np.random.randint(0, sz)
                masked_inds = masked_inds.union(set(range(start_pos, min(sz, start_pos + span_len))))

            mask[list(masked_inds)] = True

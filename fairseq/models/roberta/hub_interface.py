...
from fairseq.tokenizer import tokenize_line_by_wordpiece
...
class RobertaHubInterface(nn.Module):
    ...
    def encode(
        ...
        tokens = self.task.source_dictionary.encode_line(
            bpe_sentence,
            line_tokenizer=lambda l, vocab: ["<s>"] + [ch for ch in l[3:-4]] + ["</s>"],
            # line_tokenizer=lambda l, vocab: ["<s>"] + tokenize_line_by_wordpiece(l[3:-4], vocab) + ["</s>"],
            append_eos=False,
    ...
    def predict_tags(self, head: str, tokens: torch.LongTensor):
        ...
        return [self.task.label_dictionary[head].string([t + self.task.label_dictionary[head].nspecial]) for t in
                pred_tokens]

    def get_max_expectation_tags(self, head: str, tokens: torch.LongTensor, start_ind, end_ind):
        probs = self.predict(head, tokens)[0, 1:-1, :].softmax(dim=1).detach().numpy()
        tokens_mean_probs = np.mean(probs[start_ind:end_ind], axis=0)
        pred_token = tokens_mean_probs.argmax()
        return self.task.label_dictionary[head].string([pred_token + self.task.label_dictionary[head].nspecial])
    ...
    def fill_mask(self, masked_input: str, topk: int = 5, char_based=False):
        masked_token = "<mask>"
        if not char_based:
            assert (
            ...
            text_spans_bpe = (
                (" {0} ".format(masked_token)).join(
                    [' '.join(tokenize_line_by_wordpiece(text_span, vocab=self.task.source_dictionary.indices)) for
                     text_span in text_spans]).strip()
            )
            tokens = self.task.source_dictionary.encode_line(
                "<s> " + text_spans_bpe + " </s>",
                append_eos=False,
        ...
        text_spans = masked_input.split(masked_token)
        text_spans_bpe = []
        for ind, text_span in enumerate(text_spans):
            text_spans_bpe.extend([ch for ch in text_span])
            if ind < len(text_spans) - 1:
                text_spans_bpe.append(masked_token)
        ...
        masked_index = (tokens == self.task.mask_idx).nonzero().tolist()
        ...
        predicted_tokens = [[self.task.source_dictionary[ind] for ind
                             in l] for l in index.tolist()]
        prediction_probs = [[np.round(p, decimals=4) for p in l] for l in values.tolist()]
        inds_to_probs = []
        for i in range(len(predicted_tokens)):
            inds_to_probs.append(dict(zip(predicted_tokens[i], prediction_probs[i])))

        if topk > 1:
            return inds_to_probs

        # topk_predicted_token_bpe = self.task.source_dictionary.string(index)
        topk_predicted_token_bpe = [self.task.source_dictionary[ind.item()] for ind
                                    in index]
        ...
        for index, predicted_token_bpe in enumerate(topk_predicted_token_bpe):
            ...
            if predicted_token_bpe.startswith("\u2581"):
                predicted_token = " " + predicted_token
            if "{0}".format(masked_token) in masked_input:
                masked_input = masked_input.replace("{0}".format(masked_token),
                                                    predicted_token, 1)
                topk_filled_outputs.append(
                    (
                        values[index].item(),
        ...
        return masked_input, topk_filled_outputs

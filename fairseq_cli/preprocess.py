    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Binarizer.binarize(
                ...
                char_tokenize=args.char_tokenize,
                wordpiece_tokenize=args.wordpiece_tokenize
            )
        )
        if num_workers > 1:
            pool.join()
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos=True):
    ...
    res = Binarizer.binarize(
        filename, vocab, consumer, append_eos=append_eos, offset=offset, end=end, char_tokenize=args.char_tokenize,
        wordpiece_tokenize=args.wordpiece_tokenize
    )
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
